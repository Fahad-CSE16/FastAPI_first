...
def create_blog(request:Blog, db:Session= Depends(get_db), current_user: User = Depends(get_current_user)):
    new_blog = models.Blog(title=request.title, description = request.description, created_by_id=current_user.id)
    db.add(new_blog)
    db.commit()
    db.refresh(new_blog)
    return new_blog
...
def single_blog(id, response:Response, db:Session= Depends(get_db), current_user: User = Depends(get_current_user)):
...
def delete_single_blog(id, response:Response, db:Session= Depends(get_db), current_user: User = Depends(get_current_user)):
    blog = db.query(models.Blog).filter(models.Blog.id==id)
    if not blog.first():
        raise HTTPException(detail=f'Not found for id {id}', status_code=status.HTTP_404_NOT_FOUND)
    blog.delete()
